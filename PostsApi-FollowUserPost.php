...
if($_SERVER['REQUEST_METHOD'] == 'GET') {
      ...
      if (mysqli_num_rows($result) > 0) {
        ...
        $id_user_followed = $_GET["id_user_followed"];
        $sql = "SELECT * FROM follow_users WHERE id_user_follow = '$id_user_follow' AND id_user_following = '$id_user_followed'";
        $result2 = mysqli_query($conn, $sql);
       if (mysqli_num_rows($result2) > 0){
        
        $sql = "SELECT * FROM posts WHERE id_post_user = '$id_user_followed'";
